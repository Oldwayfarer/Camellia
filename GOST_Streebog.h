...
uint8_t Pi[256] = {0xFC, 0xEE, 0xDD, 0x11, 0xCF, 0x6E, 0x31, 0x16, 0xFB, 0xC4, 0xFA, 0xDA, 0x23, 0xC5, 0x04, 0x4D,
...
uint8_t T[64] = {0, 8, 16, 24, 32, 40, 48, 56,
...
uint8_t C_[12][64] = {{0xb1,0x08,0x5b,0xda,0x1e,0xca,0xda,0xe9,0xeb,0xcb,0x2f,0x81,0xc0,0x65,0x7c,0x1f,
...
void dbg_print(char* pr, uint8_t* a, int bytes){
    printf("%s: ", pr);
    for(int i=0; i<bytes; printf("%x ",a[i++]));
    printf("\n");
}

uint8_t* SUM512(uint8_t* a, uint8_t* b){
    uint8_t rem = 0;
    uint16_t tmp = 0;
    for(int i=63; i>=0; i--){
        tmp = a[i] + b[i] + rem;
...
uint8_t* X(uint8_t* a, uint8_t *b){
    for(int i=0; i<64; i++)
        a[i] ^= b[i];
    return a;
}

uint8_t* SPL(uint8_t* a){
    uint64_t tmp1, tmp2;
    uint8_t b[64];
    for(int i=0; i<64; a[i++] = Pi[a[i]]);
    for(int i=0; i<64; b[T[i++]] = a[i]);
    for(int i=-1; i<64; a[++i] = b[i]);
    for(int i=0; i<8; i++){
        tmp1 ^= tmp1;
        tmp2 ^= tmp2;
        for(int j=0; j<8; tmp1=(tmp1<<8)|a[i*8+j++]);
        for(int j=63; j>=0; j--)
            for(int k=63; k>=0; tmp2 ^= (((tmp1>>k)&1)&((A[63-k--]>>j)&1))<<j);
        for(int j=7; j>=0; a[i*8+j--] = (tmp2>>(8*(7-j)))&0xff);
    }
    return a;
}

uint8_t* g(uint8_t* N, uint8_t* m, uint8_t* h){
    uint8_t res[64];
    uint8_t* K = (uint8_t*)malloc(sizeof(uint8_t)*64);
    for(int i=0; i<64; res[i++] = N[i]);
    SPL(X(res, h));
    for(int i=-1; i<64; K[++i] = res[i]);
    X(res, m);
    for(int i=0; i<12; i++){
        SPL(res);
        X(res, SPL(X(K, C_[i])));
...
uint8_t* Streebog(uint8_t* text, uint32_t size,int mode){
    uint8_t N[64], Sig[64], m[64], null[64], C512[64], C2512[64];
    uint8_t* h = (uint8_t*)malloc(64*sizeof(uint8_t));
    if(h==NULL){
        printf("Malloc error\n");
        exit(0);
    }
    memset(h, mode ? 0:1, 64);
    memset(N, 0, 64);
    memset(Sig, 0, 64);
    memset(m, 0, 64);
    memset(null, 0, 64);
    ...
    while(size>=64){
        g(N, &text[size-64], h);
        SUM512(N, C2512);
        SUM512(Sig, &text[size-64]);
        size-=64;
    }
    uint64_t size_ = size*8;
    for(int i=0; i<8; i++)
        C512[63-i] = (size_>>i*8)&0xff;
    for(int i=0; i<size; m[64-size+i++]=text[i]);
    m[63-size] = 1;
    g(N, m, h);
    SUM512(N, C512);
    SUM512(Sig, m);
